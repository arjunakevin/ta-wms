            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{ location.code }}</li>
                    </ol>
                </nav>
                <h4 class="mg-b-0 tx-spacing--1">{{ location.code }}</h4>
            </div>
            <div class="d-none d-md-block">
                <Link :href="$route('locations.edit', location)" as="button" type="button" class="btn btn-sm pd-x-15 btn-white btn-uppercase mg-l-5"><i data-feather="edit" class="wd-10 mg-r-5"></i> Edit</Link>
                ...
                <form>
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10 d-flex align-items-center">
                            <span class="tx-bold">{{ location.id }}</span>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="code" class="col-sm-2 col-form-label">Location Code</label>
                        <div class="col-sm-10 d-flex align-items-center">
                            <span class="tx-bold">{{ location.code }}</span>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-form-label col-sm-2 pt-0">Pick Blocked</label>
                        <div class="col-sm-10">
                            <span class="badge badge-danger" v-if="location.pick_blocked">Yes</span>
                            <span class="badge badge-success" v-else>No</span>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-form-label col-sm-2 pt-0">Put Blocked</label>
                        <div class="col-sm-10">
                            <span class="badge badge-danger" v-if="location.put_blocked">Yes</span>
                            <span class="badge badge-success" v-else>No</span>
                        </div>
                    </div>
                </form>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content tx-14">
                    <div class="modal-header">
                        <h6 class="modal-title" id="exampleModalLabel2">Delete Location</h6>
...
export default {
    ...
    methods: {
        submitDeletion() {
            this.$inertia.delete(this.$route('locations.destroy', this.location), {
                onBefore: () => {
                    this.deleting = true;
