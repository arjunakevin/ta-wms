<template>
    <div>
            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">{{ inbound.reference }}</li>
                    </ol>
                </nav>
                <h4 class="mg-b-0 tx-spacing--1">{{ inbound.reference }}</h4>
            </div>
            <div class="d-none d-md-block">
                <Link :href="$route('inbounds.edit', inbound)" as="button" type="button" class="btn btn-sm pd-x-15 btn-primary btn-uppercase mg-l-5"><i data-feather="edit" class="wd-10 mg-r-5"></i> Edit</Link>
        ...
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            <li class="nav-item">
                <a class="nav-link active">Inbound Delivery</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="#">Good Receiving</a>
            </li>
        </ul>
            ...
            <div class="tab-pane fade show active" id="home" role="tabpanel" aria-labelledby="home-tab">
                <form>
                    <div class="form-group row">
                        <label for="id" class="col-sm-2 col-form-label">Inbound ID</label>
                        <div class="col-sm-10 d-flex align-items-center">
                            <span class="tx-bold">{{ inbound.id }}</span>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="reference" class="col-sm-2 col-form-label">Inbound Reference</label>
                        <div class="col-sm-10 d-flex align-items-center">
                            <span class="tx-bold">{{ inbound.reference }}</span>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="client_code" class="col-sm-2 col-form-label">Client</label>
                        <div class="col-sm-10 d-flex align-items-center">
                            <span>{{ inbound.client.name }} (<span class="tx-bold">{{ inbound.client.code }}</span>)</span>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="arrival_date" class="col-sm-2 col-form-label">Arrival Date</label>
                        <div class="col-sm-10 d-flex align-items-center">
                            <span class="tx-bold">{{ inbound.arrival_date || '-' }}</span>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="po_date" class="col-sm-2 col-form-label">PO Date</label>
                        <div class="col-sm-10 d-flex align-items-center">
                            <span class="tx-bold">{{ inbound.po_date || '-' }}</span>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="notes" class="col-sm-2 col-form-label">Notes</label>
                        <div class="col-sm-10 d-flex align-items-center">
                            <span class="tx-bold">{{ inbound.notes || '-' }}</span>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-form-label col-sm-2 pt-0">Status</label>
                        <div class="col-sm-10" v-html="getInboundStatusLabel(inbound.status)">
                        </div>
                    </div>
                    <hr>
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content tx-14">
                    <div class="modal-header">
                        <h6 class="modal-title" id="exampleModalLabel2">Delete Inbound Delivery</h6>
...
export default {
    ...
    methods: {
        submitDeletion() {
            this.$inertia.delete(this.$route('inbounds.destroy', this.inbound), {
                onBefore: () => {
                    this.deleting = true;
